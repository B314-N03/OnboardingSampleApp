...
function TenantSetup() {
  ...
  const loadTenant = useCallback(async (customerId) => {
    ...
    const res = await apiFetch(`/api/tenants/${customerId}`);
    if (res.status === 404) {
      setTenant(null);
      return;
    }
  ...
  const updateTenant = async (updates) => {
    const res = await apiFetch(`/api/tenants/${selectedId}`, {
      method: 'PUT',
  ...
  const provision = async () => {
    ...
    try {
      ...
      const stepRes = await apiFetch(`/api/customers/${selectedId}/steps/step_3`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
